...
public class KarafRuntimeDataProvider extends AbstractRuntimeDataProvider {
    ...
    private IStatus loadBundleData(IProgressMonitor monitor) {
        ...
        if (!mbeanProvider.isOpen()) {
            return new Status(IStatus.WARNING, KarafUIPluginActivator.PLUGIN_ID, "Connection to MBean server has been closed");
        }
        ...
        try {
            ...
            return new Status(IStatus.ERROR, KarafUIPluginActivator.PLUGIN_ID, "Unable to connect to MBeanServer", e);
        } catch (Exception e) {
            KarafUIPluginActivator.getLogger().warn("Unable to update OSGi Bundles", e);
        }
    ...
    private IStatus loadServiceData(IProgressMonitor monitor) {
        ...
        try {
            ...
            KarafUIPluginActivator.getLogger().warn("Unable to update OSGi Services", e);
        }

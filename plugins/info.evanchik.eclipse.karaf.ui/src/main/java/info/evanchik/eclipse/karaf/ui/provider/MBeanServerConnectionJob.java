...
public class MBeanServerConnectionJob extends Job {
    ...
    protected IStatus run(IProgressMonitor monitor) {
        ...
        try {
            ...
            status = new Status(IStatus.OK, KarafUIPluginActivator.PLUGIN_ID, "Successfully connected to JMX MBeanServer for: "
                    + getName());
            ...
            if (retries < maxRetries) {
                ...
                status = new Status(IStatus.WARNING, KarafUIPluginActivator.PLUGIN_ID, "Retrying connection for for: " + getName(), ex);

                schedule(rescheduleDelay);
            } else {
                status = new Status(IStatus.ERROR, KarafUIPluginActivator.PLUGIN_ID, "Unable to connect to: " + getName());
            }
